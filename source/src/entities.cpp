...
void checkitems(playerent *d)
{
    ...
    if (m_flags)
        loopi(2)
        {
            flaginfo &f = flaginfos[i];
            entity &e = *f.flagent;
            if (!e.spawned || !f.ack || (f.state == CTFF_INBASE && !numflagspawn[i]))
                continue;
            if (OUTBORD(f.pos.x, f.pos.y))
                continue;
...
bool selectnextprimary(int num)
{
    ...
    case GUN_ASSAULT:
        ...
        addmsg(SV_PRIMARYWEAP, "ri", 82); // EGK: Changing this to a non-weapon ID would crash the server.
        return true;
...
VARFP(nextprimary, 0, GUN_ASSAULT, NUMGUNS,
      {
          if (!selectnextprimary(nextprimary))
              selectnextprimary((nextprimary = GUN_ASSAULT));
      });
    ...
    loopi(MAXENTTYPES)
    {
        if (entcnt[i])
            ...
            case MAPMODEL:
                ...
                break;
            case PLAYERSTART:
                conoutf(" %d %s, %d CLA, %d RVSF, %d FFA", entcnt[i], entnames[i], spawncnt[0], spawncnt[1], spawncnt[2]);
                break;
            case CTF_FLAG:
                conoutf(" %d %s, %d CLA, %d RVSF", entcnt[i], entnames[i], spawncnt[3], spawncnt[4]);
                break;
            default:
