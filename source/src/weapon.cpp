...
{
    if (!w || !player1->weaponsel->deselectable())
        return;
    if (w->selectable())
    {
        int i = w->type;
        ...
        if (w->type == GUN_PISTOL && akimbo->selectable())
            w = akimbo;
...
void requestweapon(int *w)
{
    ...
    {
        if (player1->akimbo && *w == GUN_PISTOL)
            *w = GUN_AKIMBO;
...
void shiftweapon(int *s)
{
    ...
    {
        if (!player1->weaponsel->deselectable())
            return;
        ...
        if (player1->akimbo)
        {
            ...
            if (curweapon->type == GUN_PISTOL)
                curweapon = akimbo; // fix selection
        }
...
void quicknadethrow(bool on)
{
    ...
    if (on)
    {
        if (player1->weapons[GUN_GRENADE]->mag > 0)
        {
            ...
            if (player1->weaponsel->type == GUN_GRENADE || quicknade_hold)
            {
...
{
    if (!p || p->state != CS_ALIVE || p->weaponsel->reloading || p->weaponchanging)
        return;
...
static inline bool intersectbox(const vec &o, const vec &rad, const vec &from, const vec &to, vec *end) // if lineseg hits entity bounding box
{
    ...
    if (p->x <= o.x + rad.x && p->x >= o.x - rad.x && p->y <= o.y + rad.y && p->y >= o.y - rad.y && p->z <= o.z + rad.z && p->z >= o.z - rad.z)
    {
        if (end)
            *end = *p;
        return true;
...
int intersect(playerent *d, const vec &from, const vec &to, vec *end)
{
    ...
    if (d->head.x >= 0)
    {
        ...
        {
            if (end)
                (*end = to).sub(from).mul(dist).add(from);
            return 2;
        }
    }
...
playerent *intersectclosest(const vec &from, const vec &to, playerent *at, float &bestdist, int &hitzone, bool aiming = true)
{
    ...
    loopv(players)
    {
        ...
        if (!o || o == at || (o->state != CS_ALIVE && (aiming || (o->state != CS_EDITING && o->state != CS_LAGGED))))
            continue;
    ...
    if (d->weaponsel->type == GUN_SHOTGUN)
    {
            ...
            loopi(SGRAYS)
        {
            ...
            if (hits[h])
            {
                ...
                if (numhits)
                    hitpush(dmgreal, o, d, from, to, d->weaponsel->type, lastsgs_hits == SGRAYS * 3, isbigendian() ? endianswap(info) : info);
...
void weapon::renderhudmodel(int lastaction, int index)
{
    ...
    weaponmove wm;
    if (!intermission)
        wm.calcmove(unitv, lastaction, p);
...
{
    if (gunwait)
        gunwait = max(gunwait - (millis - owner->lastaction), 0);
}
...
{
    if (nadestate != NS_ACTIVATED && nadestate != NS_THROWN)
        return;
...
{
    if (quicknade && owner->weaponsel->type == GUN_GRENADE)
...
{
    reset();
    audiomgr.playsound(S_GUNCHANGE);
}
void grenades::onownerdies()
{
    reset();
    if (owner == player1 && inhandnade)
        dropnade();
}
...
{
    if (autoreload && owner == player1 && !mag)
        reload(true);
}
...
void sniperrifle::setscope(bool enable)
{
    ...
    {
        if (scoped == false && enable == true)
            scoped_since = lastmillis;
        if (enable != scoped)
            owner->scoping = enable;
        scoped = enable;
    }
}
...
{
    if (player1->weaponsel->type != GUN_CPISTOL)
        return;
    if (intermission)
        return;
    ...
    if (!burst)
    {
        if (enable && burstshots == 0)
            attack(true);
    }
    else
    {
        if (burstshots == 0)
            burst = player1->attacking = enable;
    }
}
...
void akimbo::onammopicked()
{
    ...
    if (owner == player1)
    {
        ...
        if (akimboautoswitch || owner->weaponsel->type == GUN_PISTOL)
            owner->weaponswitch(this); // Give the client full control over akimbo auto-switching // Bukz 2011apr23
...
bool knife::attack(vec &targ)
{
    ...
    to.add(unitv);
    if (owner->pitch < 0)
        to.z += 2.5 * sin(owner->pitch * 0.01745329);
...
COMMANDF(
    setscope, "i", (int *on) { setscope(*on != 0); });

void shoot(playerent *p, vec &targ)
{
    if (p->state != CS_ALIVE || p->weaponchanging)
        return;
