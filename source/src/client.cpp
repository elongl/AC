...
bool multiplayer(bool msg)
{
    ...
    if (curpeer && msg)
        conoutf(_("operation not available in multiplayer"));
    return curpeer != NULL;
}
...
bool allowedittoggle()
{
    ...
    if (!allow)
        conoutf(_("editing in multiplayer requires coopedit mode (1)"));
    return allow;
}
...
void _toserver(char *text, int msg, int msgt)
{
    ...
    if (servstate.mastermode == MM_MATCH && servstate.matchteamsize && !team_isactive(player1->team) && !(player1->team == TEAM_SPECT && player1->clientrole == CR_ADMIN))
...
void toserverme(char *text)
{
    ...
    // func call
    char dbgtext[] = "aaaabbbbccccddddeeeeffffgggghhhhiiiijjjjkkkkllllmmmmnnnnooooppppqqqqrrrrssssttttuuuuvvvvwwwwxxxxyyyyzzzzAAAABBBBCCCCDDDDEEEEFFFFGGGGHHHHIIIIJJJJKKKKLLLLMMMMNNNNOOOOPPPPQQQQRRR"
                     "paD";
...
void c2sinfo(playerent *d) // send update to the server
{
    ...
    if (d->state == CS_ALIVE || d->state == CS_EDITING)
    {
        ...
        { // compact POS packet
            ...
            b.putbits(1, z < 0 ? 1 : 0);
            if (z < 0)
                z = -z; // z is encoded with 3..10 bits minimum (fitted to byte boundaries), or full 11 bits if necessary
        ...
        case ENET_EVENT_TYPE_DISCONNECT:
        {
            ...
            else
            {
                if (!discmillis || event.data)
                    conoutf(_("%c3server network error, disconnecting (%s) ..."), CC, disc_reason(event.data));
                disconnect();
            }
            return;
        }
...
void genauthkey(const char *secret)
{
    if (!secret[0])
    {
        conoutf("you must specify a secret password");
        return;
    }
...
void saveauthkeys()
{
    if (authkeys.length())
    {
        ...
        if (!f)
        {
            conoutf("failed to open config/auth.cfg for writing");
            return;
        }
        loopv(authkeys)
        {
        ...
        conoutf("you need to use 'addauthkey USER KEY DESC' first; one DESC 'public', one DESC empty(=private)");
}
...
COMMANDF(
    hasauthkey, "ss", (char *name, char *desc) { intret(_hasauthkey(name, desc) ? 1 : 0); });
COMMAND(genauthkey, "s");
COMMAND(saveauthkeys, "");
COMMANDF(
    auth, "s", (char *desc) { intret(tryauth(desc)); });
...
void getdemo(int *idx, char *dsp)
{
    ...
    if (*idx <= 0)
        conoutf(_("getting demo..."));
    else
        conoutf(_("getting demo %d..."), *idx);
...
void setupcurl()
{
    if (curl_global_init(CURL_GLOBAL_NOTHING))
        conoutf(_("\f3could not init cURL, content downloads not available"));
    else
    {
        havecurl = true;
...
int pingpckservers(void *data)
{
    ...
    loopv(pckservers)
    {
        ...
        if (serv->ping > 0)
            conoutf("%d. %s (%d ms)", i + 1, serv->addr, serv->ping);
        else
            conoutf("%d. %s (did not reply)", i + 1, serv->addr);
    }
    SDL_mutexV(pingpcksrvlock);
...
int processdownload(package *pck)
{
    ...
    if (!pck->pending)
    {
        ...
        case PCK_AUDIO:
        {
            ...
            if (!copyfile(tmpname, pckname))
                conoutf(_("\f3failed to install"), pckname);
            break;
        }
...
double dlpackage(package *pck)
{
    ...
    if (result == CURLE_OPERATION_TIMEDOUT || result == CURLE_COULDNT_RESOLVE_HOST)
    {
        ...
        loopv(pckservers) if (pckservers[i]->responsive)
        {
            source = pckservers[i];
            break;
        }
    ...
    if (!result && httpresult == 200)
        ...
        conoutf(_("\f3download cancelled"));
    else
        conoutf(_("\f2request for %s failed (cURL %d, HTTP %d)"), req, result, httpresult);
    ...
    enumerate(pendingpackages, package *, pck,
              {
                  ...
                  {
                      if (connected)
                          c2skeepalive(); // try to avoid time out
...
{
    if (!havecurl || canceldownloads || type < 0 || type >= PCK_NUM || pendingpackages.access(path))
        return false;
