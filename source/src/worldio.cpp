...
bool load_world(char *mname) // still supports all map formats that have existed since the earliest cube betas!
{
    ...
    if (f->read(&tmp, sizeof(header) - sizeof(int) * 16) != sizeof(header) - sizeof(int) * 16)
    {
        conoutf("\f3while reading map: header malformatted");
        delete f;
        return false;
    }
    lilswap(&tmp.version, 4);
    if (strncmp(tmp.head, "CUBE", 4) != 0 && strncmp(tmp.head, "ACMP", 4) != 0)
    {
        conoutf("\f3while reading map: header malformatted");
        delete f;
        return false;
    ...
    if (tmp.version >= 4 && f->read(&tmp.waterlevel, sizeof(int) * 16) != sizeof(int) * 16)
    {
        conoutf("\f3while reading map: header malformatted");
        delete f;
        return false;
    }
    if ((tmp.version == 7 || tmp.version == 8) && !f->seek(sizeof(char) * 128, SEEK_CUR))
    {
        conoutf("\f3while reading map: header malformatted");
        delete f;
        return false;
    ...
    loopk(cubicsize) if (world[k].type != SOLID)
    {
        ...
        if (cwy > mapdims[3])
            mapdims[3] = cwy;
        if (world[k].floor != -128 && world[k].floor < mapdims[6])
            mapdims[6] = world[k].floor;
        if (world[k].ceil > mapdims[7])
            mapdims[7] = world[k].ceil;
    }
    ...
    if (downloaded > 0)
        printf("downloaded content (%d KB in %d seconds)\n", downloaded / 1000, watch.stop() / 1000);
