...
void setupdemoplayback()
{
    ...
    else
    {
        ...
        if (hdr.version != DEMO_VERSION)
            formatstring(msg)("demo \"%s\" requires an %s version of AssaultCube", file, hdr.version < DEMO_VERSION ? "older" : "newer");
        else if (hdr.protocol != PROTOCOL_VERSION && !(hdr.protocol < 0 && hdr.protocol == -PROTOCOL_VERSION) && hdr.protocol != 1132)
            formatstring(msg)("demo \"%s\" requires an %s version of AssaultCube", file, hdr.protocol < PROTOCOL_VERSION ? "older" : "newer");
        else if (hdr.protocol == 1132)
            sendservmsg("WARNING: using experimental compatibility mode for older demo protocol, expect breakage");
...
void flagaction(int flag, int action, int actor)
{
    ...
    if (m_ctf || m_htf)
    {
        ...
        case FA_STEAL:
        {
            if (deadactor || f.state != (action == FA_STEAL ? CTFF_INBASE : CTFF_DROPPED) || !flagdistance(f, actor))
            {
                abort = 10;
                break;
            }
        ...
        case FA_SCORE: // ctf: f = carried by actor flag,  htf: f = hunted flag (run over by actor)
            if (m_ctf)
            {
                if (f.state != CTFF_STOLEN || f.actor_cn != actor || of.state != CTFF_INBASE || !flagdistance(of, actor))
                {
                    abort = 14;
                    break;
                }
    ...
    if (score)
    {
        ...
        if (m_teammode)
            computeteamwork(c->team, c->clientnum); /** WIP */
    }
...
void distributeteam(int team)
{
    ...
    if (!numsp)
        numsp = 30; // no spawns: try to distribute anyway
    twoint ti;
    tdistrib.shrink(0);
    loopv(clients) if (clients[i]->type != ST_EMPTY)
    {
...
void arenacheck()
{
    ...
    {
        client &c = *clients[i];
        if (c.type == ST_EMPTY || !c.isauthed || !c.isonrightmap || team_isspect(c.team))
            continue; /// TODO: simplify the team/state sysmtem, it is not smart to have SPECTATE in both, for example
    ...
    if (!dead || gamemillis < lastdeath + 500)
        return;
    items_blocked = true;
    sendf(-1, 1, "ri2", SV_ARENAWIN, alive ? alive->clientnum : -1);
    arenaround = gamemillis + 5000;
    if (autoteam && m_teammode)
        refillteams(true);
}
...
{
    if (!valid_client(sender) || clients[sender]->team == TEAM_VOID)
        return;
    packetbuf p(MAXTRANS, ENET_PACKET_FLAG_RELIABLE);
    putint(p, msgtype);
    putint(p, sender);
...
bool serverpickup(int i, int sender) // server side item pickup, acknowledge first client that gets it
{
    ...
    if (!e.spawned)
    {
        if (!e.legalpickup && hn && !m_demo)
            logline(ACLOG_INFO, "[%s] tried to pick up entity #%d (%s) - can't be picked up in this gamemode or at all", hn, i, entnames[e.type]);
        return false;
    }
    if (sender >= 0)
    {
...
void serverdamage(client *target, client *actor, int damage, int gun, bool gib, const vec &hitpush = vec(0, 0, 0))
{
    ...
    if (ts.health <= 0)
    {
        ...
        if (!suic)
            logline(ACLOG_INFO, "[%s] %s %s%s %s", actor->hostname, actor->name, killmessage(gun, gib), tk ? " their teammate" : "", target->name);
...
bool updateclientteam(int cln, int newteam, int ftr)
{
    if (!valid_client(cln))
        return false;
    if (!team_isvalid(newteam) && newteam != TEAM_ANYACTIVE)
        newteam = TEAM_SPECT;
    client &cl = *clients[cln];
    if (cl.team == newteam && ftr != FTR_AUTOTEAM)
...
{
    if (mastermode != MM_OPEN || totalclients < 3)
        return true;
    ...
    if (2 * tscore[h] < 3 * tscore[l] || totalscore < nplayers * 100)
        return true;
    ...
    { // the h score team has less or the same player number, so, lets exchange
        loopv(clients) if (clients[i]->type != ST_EMPTY)
        {
            client *c = clients[i]; // loop for h
            if (c->isauthed && c->team == h && !c->state.forced && clienthasflag(i) < 0)
...
bool refillteams(bool now, int ftr) // force only minimal amounts of players
{
    ...
    if (lasttime_eventeams > gamemillis)
        lasttime_eventeams = 0;
    if (diffnum > 1)
    {
        if (now || gamemillis - lasttime_eventeams > 8000 + allplayers * 1000 || diffnum > 2 + allplayers / 10)
        {
            ...
            while (diffnum > 1 && moveable[bigteam] > 0)
            {
                ...
                loopv(clients) if (clients[i]->type != ST_EMPTY && !clients[i]->at3_dontmove) // try all still movable players
                {
                    ...
                    if (fit < 0)
                        fit = -(fit * 15) / 10; // avoid too good players
...
void startgame(const char *newname, int newmode, int newtime, bool notify)
{
    ...
    else
    {
        ...
        mapbuffer.clear();
        if (isdedicated && distributablemap(maploc))
            mapbuffer.load();
        if (ms)
        {
            smapstats = *ms;
            ...
            loopi(smapstats.hdr.numents)
            {
                ...
                sents.add(se);
                if (e.fitsmode(smode))
                    sents[i].spawned = sents[i].legalpickup = true;
            }
            ...
            sendservmsg("\f3server error: map not found - please start another map or send this map to the server");
        if (notify)
        {
            // change map
        ...
        if (ms)
            concatformatstring(gsmsg, "(map rev %d/%d, %s, 'getmap' %sprepared)", smapstats.hdr.maprevision, smapstats.cgzsize, maplocstr[maploc], mapbuffer.available() ? "" : "not ");
        else
...
int getbantype(int cn)
{
    ...
    if (checkgban(c.peer->address.host))
        return BAN_MASTER;
    if (ipblacklist.check(c.peer->address.host))
    ...
    loopv(bans)
    {
        ...
        if (b.address.host == c.peer->address.host)
            return b.type;
    }
    return BAN_NONE;
}
...
struct voteinfo
{
    ...
    {
        if (result != VOTE_NEUTRAL)
            return; // block double action
        if (action && !action->isvalid())
            end(VOTE_NO);
        int stats[VOTE_NUM] = {0};
        int adminvote = VOTE_NEUTRAL;
        loopv(clients) if (clients[i]->type != ST_EMPTY /*&& clients[i]->connectmillis < callmillis*/) // new connected people will vote now
        {
            stats[clients[i]->vote]++;
            if (clients[i]->role == CR_ADMIN)
                adminvote = clients[i]->vote;
        };
...
bool scallvote(voteinfo *v, ENetPacket *msg) // true if a regular vote was called
{
    ...
    else if ((c->lastvotecall && servmillis - c->lastvotecall < 60 * 1000 && c->role != CR_ADMIN && numclients() > 1) || c->nvotes > 3)
        error = VOTEE_MAX;
    else if (((v->boot == 1 && c->role < roleconf('w')) || (v->boot == 2 && c->role < roleconf('X'))) && !is_lagging(b) && !b->mute && b->spamcount < 2)
    {
    ...
    {
        if (v->type == SA_MAP && v->num1 >= GMODE_NUM)
            map_queued = true;
        if (!v->gonext)
            sendpacket(-1, 1, msg, v->owner); // FIXME in fact, all votes should go to the server, registered, and then go back to the clients
        else
            callvotepacket(-1, v); // also, no vote should exclude the caller... these would provide many code advantages/facilities
...
{
    pwddetail pd;
    if (!isdedicated || !valid_client(client))
        return;
    pd.line = -1;
...
{
    if (!clients.inrange(n) || clients[n]->type != ST_TCPIP)
        return;
    sdropflag(n);
    client &c = *clients[n];
    ...
    if (reason >= 0)
        logline(ACLOG_INFO, "[%s] disconnecting client %s (%s) cn %d, %d seconds played%s", c.hostname, c.name, disc_reason(reason), n, sp, scoresaved);
    else
        logline(ACLOG_INFO, "[%s] disconnected client %s cn %d, %d seconds played%s", c.hostname, c.name, n, sp, scoresaved);
    totalclients--;
    c.peer->data = (void *)-1;
    if (reason >= 0)
    ...
    if (curvote)
        curvote->evaluate();
    if (*scoresaved && mastermode == MM_MATCH)
        senddisconnectedscores(-1);
}
...
{
    if (!valid_client(receiver))
        return;
    packetbuf p(MAXTRANS, ENET_PACKET_FLAG_RELIABLE);
    putint(p, SV_IPLIST);
    loopv(clients) if (valid_client(i) && clients[i]->type == ST_TCPIP && i != receiver && (clients[i]->clientnum == cn || cn == -1))
    {
        putint(p, i);
...
void putinitclient(client &c, packetbuf &p)
{
    ...
    enet_uint32 ip = 0;
    if (c.type == ST_TCPIP)
        ip = c.peer->address.host & 0xFFFFFF;
...
void welcomeinitclient(packetbuf &p, int exclude = -1)
{
    ...
    {
        client &c = *clients[i];
        if (c.type != ST_TCPIP || !c.isauthed || c.clientnum == exclude)
            continue;
        putinitclient(c, p);
    }
}
...
void welcomepacket(packetbuf &p, int n)
{
    ...
    if (c)
    {
        if (c->type == ST_TCPIP && serveroperator() != -1)
            sendserveropinfo(n);
    ...
    if (!c || clients.length() > 1)
    {
        ...
        {
            client &c = *clients[i];
            if (c.type != ST_TCPIP || c.clientnum == n)
                continue;
            putint(p, c.clientnum);
...
void process(ENetPacket *packet, int sender, int chan)
{
    ...
    if (cl && !cl->isauthed)
    {
        ...
        else
        {
            ...
            if (wl == NWL_PASS)
                concatstring(tags, ", nickname whitelist match");
            if (wl == NWL_UNLISTED)
                bl = nickblacklist.checkblacklist(cl->name);
            ...
            {                                                                                                                                                                                                   // admin (or deban) password match
                cl->isauthed = true;
                if (!pd.denyadmin && wantrole == CR_ADMIN)
                    clientrole = CR_ADMIN;
                if (bantype == BAN_VOTE)
                {
        ...
        if (clientrole != CR_DEFAULT)
            changeclientrole(sender, clientrole, NULL, true);
    ...
    while ((curmsg = p.length()) < p.maxlen)
    {
        ...
        case SV_VOICECOMTEAM:
        {
            ...
            if (servmillis > cl->mute) // client is not muted
            {
                ...
                if (m_teammode)
                    checkteamplay(s, sender); // finally here we check the teamplay comm
                if (type == SV_VOICECOM)
                {
                    QUEUE_MSG;
                }
        ...
        case SV_MAPIDENT:
        {
            ...
            { // here any game really starts for a client: spawn, if it's a new game - don't spawn if the game was already running
                ...
                if (cl->loggedwrongmap)
                    logline(ACLOG_INFO, "[%s] %s is now on the right map: revision %d/%d", cl->hostname, cl->name, rev, gzs);
                bool spawn = false;
                ...
                {
                    if ((cl->freshgame || numclients() < 2) && !m_demo)
                        spawn = true;
                }
                cl->freshgame = false;
                if (spawn)
        ...
        case SV_WEAPCHANGE:
        {
            ...
            if (gunselect < 0 || gunselect >= NUMGUNS || gunselect == GUN_CPISTOL)
                break;
            if (!m_demo && !m_coop)
                checkweapon(type, gunselect);
            ...
            if (namechanged)
            {
                ...
                if (servmillis - cl->lastprofileupdate < 1000)
                {
                    ...
                    if (cl->fastprofileupdates == 3)
                        sendservmsg("\f3Please do not spam");
        ...
        case SV_SPAWN:
        {
            ...
            if ((cl->state.state != CS_ALIVE && cl->state.state != CS_DEAD && cl->state.state != CS_SPECTATE) ||
                ls != cl->state.lifesequence || cl->state.lastspawn < 0 || gunselect < 0 || gunselect >= NUMGUNS || gunselect == GUN_CPISTOL)
                break;
...
#define seteventmillis(event)                                                                 \
    {                                                                                         \
        ...
        {                                                                                     \
            ...
            if (!m_demo && !m_coop)
                checkshoot(sender, shot, hits, tcn);
            break;
        }
        ...
        case SV_POSC:
        {
            ...
            if (((cl->f >> 6) & 1) != (cl->state.lifesequence & 1) || usefactor != (smapstats.hdr.sfactor < 7 ? 7 : smapstats.hdr.sfactor))
                break;
            cl->state.o[0] = xt / DMF;
            cl->state.o[1] = yt / DMF;
            cl->state.o[2] = zt / DMF;
        ...
        case SV_EXTENSION:
        {
            ...
            // add other extensions here
                ...
                for (; n > 0; n--)
                    getint(p); // ignore unknown extensions
    ...
    if (mastermode == MM_MATCH)
    {
        loopv(savedscores)
        {
            ...
            if (sc.valid)
            {
                ...
                if (m_flags)
                    concatformatstring(text, "%4d ", sc.flagscore);
...
void linequalitystats(int elapsed)
{
    ...
    { // collect data
        ...
        loopv(clients)
        {
            ...
            if (throttle < 11)
                c2++;
            if (rtt > c.bottomRTT * 2 && rtt - c.bottomRTT > 300)
                r1++;
        }
...
void extping_serverinfo(ucharbuf &pi, ucharbuf &po)
{
    ...
    if (!buf)
        buf = infofiles.getinfo(reslang = "en"); // try english
...
void extinfo_statsbuf(ucharbuf &p, int pid, int bpos, ENetSocket &pongsock, ENetAddress &addr, ENetBuffer &buf, int len, int *csend)
{
    loopv(clients)
    {
        if (clients[i]->type != ST_TCPIP)
            continue;
        if (pid > -1 && clients[i]->clientnum != pid)
            continue;
...
void initserver(bool dedicated, int argc, char **argv)
{
    ...
    string identity;
    if (scl.logident[0])
        filtertext(identity, scl.logident, 0);
    else
    ...
    if ((isdedicated = dedicated))
    {
        ...
        if (scl.servdesc_pre[0] || scl.servdesc_suf[0])
            logline(ACLOG_VERBOSE, "custom server description: \"%sCUSTOMPART%s\"", scl.servdesc_pre, scl.servdesc_suf);
