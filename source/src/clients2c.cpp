...
bool changemapserv(char *name, int mode, int download, int revision) // forced map change from the server
{
    ...
    if (download > 0)
    {
        ...
        {
            if (!revmatch)
                conoutf(_("%c3demo was recorded on map revision %d, you have map revision %d"), CC, revision, hdr.maprevision);
        }
        else
        {
            if (securemapcheck(name, false))
                return true;
            bool sizematch = maploaded == download || download < 10;
            if (loaded && sizematch && revmatch)
                return true;
            ...
            else
            {
                if (!loaded || download < 10)
                    conoutf(_("\"getmap\" to download the current map from the server"));
                else
                    conoutf(_("\"getmap\" to download a %s version of the current map from the server"),
...
void parsepositions(ucharbuf &p)
{
    ...
    while (p.remaining())
        ...
        case SV_POSC:
        {
            ...
            if (!d || seqcolor != (d->lifesequence & 1))
                continue;
            vec oldpos(d->o);
            ...
            loopi(3)
            {
                ...
                d->vel.v[i] += vel.v[i];
                if (i == 2 && d->onfloor && d->vel.v[i] < 0.0f)
                    d->vel.v[i] = 0.0f;
            }
            d->o = o;
            d->o.z += d->eyeheight;
...
#undef DIST
    ...
    {
        entity &e1 = ents[i];
        if (e1.type < I_CLIPS || e1.type > I_AKIMBO)
            continue;
        float density = 0, hdensity = 0;
        loopvj(ents)
        {
            entity &e2 = ents[j];
            if (e2.type < I_CLIPS || e2.type > I_AKIMBO || i == j)
                continue;
...
#undef DIST
            if (r2 == 0.0f)
            {
                conoutf("\f3MAP CHECK FAIL: Items too close %s %s (%hd,%hd)", entnames[e1.type], entnames[e2.type], e1.x, e1.y);
                item_fail = true;
                break;
            }
            r2 = 1 / r2;
        ...
        if (hdensity > 0.5f)
        {
            conoutf("\f3MAP CHECK FAIL: Items too close %s %.2f (%hd,%hd)", entnames[e1.type], hdensity, e1.x, e1.y);
            item_fail = true;
            break;
        }
        switch (e1.type)
...
#define LOGTHISSWITCH(X)                                                                                         \
    if (density > X)                                                                                             \
    {                                                                                                            \
        conoutf("\f3MAP CHECK FAIL: Items too close %s %.2f (%hd,%hd)", entnames[e1.type], density, e1.x, e1.y); \
        item_fail = true;                                                                                        \
        break;                                                                                                   \
    }
        case I_CLIPS:
...
void showhudextras(char hudextras, char value)
{
    ...
    case HE_COMBO5:
    {
        ...
        if (n > 3)
            outf("\f3%s", strcaps("monster combo!!!", caps)); // I expect to never see this one
        else if (!n)
...
void parsemessages(int cn, playerent *d, ucharbuf &p, bool demo = false)
{
    ...
    while (p.remaining())
    {
        ...
        {
            if (type > SV_IPLIST)
                --type; // SV_WHOIS removed
            if (type >= SV_TEXTPRIVATE)
                ++type;                // SV_TEXTPRIVATE added
        ...
        case SV_SERVINFO: // welcome message from the server
        {
            ...
            if (prot != CUR_PROTOCOL_VERSION && !(watchingdemo && prot == -PROTOCOL_VERSION))
            {
                ...
                if (watchingdemo)
                    conoutf("breaking loop : \f3this demo is using a different protocol\f5 : end it now!"); // SVN-WiP-bug: causes endless retry loop else!
                else
                    disconnect();
                return;
            }
            ...
            if (getint(p) > 0)
                conoutf(_("INFO: this server is password protected"));
            sendintro();
            break;
        }
        ...
        case SV_CDIS:
        {
            ...
            if (!d)
                break;
            if (d->name[0])
                conoutf(_("player %s disconnected"), colorname(d));
        ...
        case SV_SPAWNSTATE:
        {
            ...
            if (player1->lifesequence == 0)
                player1->resetstats(); //NEW
            break;
        }
        ...
        case SV_EDITENT: // coop edit of ent
        {
            ...
            if (ents[i].type == LIGHT || to == LIGHT)
                calclight();
            if (ents[i].type == SOUND)
                audiomgr.preloadmapsound(ents[i]);
            break;
        }
        ...
        case SV_ARENAWIN:
        {
            ...
            if (m_botmode && acn == -2)
                hudoutf(_("the bots have won the round!"));
            else if (!alive)
                hudoutf(_("everyone died!"));
            else if (m_teammode)
                hudoutf(_("team %s has won the round!"), team_string(alive->team));
            else if (alive == player1)
                hudoutf(_("you are the survivor!"));
            else
                hudoutf(_("%s is the survivor!"), colorname(alive));
        ...
        case SV_SPAWNDENY:
        {
            ...
            if (spawnpermission == SP_REFILLMATCH)
                hudoutf("\f3You can now spawn to refill your team.");
            break;
        }
        case SV_FORCEDEATH:
        {
        ...
        case SV_SERVOPINFO:
        {
            ...
            {
                if (players[i])
                    players[i]->clientrole = CR_DEFAULT;
            }
            ...
            if (cl >= 0 && r >= 0)
            {
                ...
                if (pl)
                {
                    ...
                    if (pl->name[0])
                    {
                        ...
                        if (pl == player1)
                            conoutf(_("you claimed %s status"), r == CR_ADMIN ? "admin" : "master");
                        else
                            conoutf(_("%s claimed %s status"), colorname(pl), r == CR_ADMIN ? "admin" : "master");
                    }
                }
            }
            break;
        ...
        case SV_TEAMDENY:
        {
            ...
            {
                if (team_isvalid(t))
                    conoutf(_("you can't change to team %s"), team_string(t));
            }
            else
            {
                ...
                if (m_teammode || team_isspect(fnt))
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            case FTR_AUTOTEAM:
                                if (watchingdemo)
                                    conoutf(_("the server forced %s to team %s"), colorname(d), nts);
                                else
                                    hudoutf(_("the server forced %s to %s team"), colorname(d), et ? _("the enemy") : _("your"));
                                break;
                            }
                        }
                    ...
                    conoutf(_("%s changed to active play"), you ? _("you") : colorname(d));
                d->team = fnt;
        ...
        case SV_CALLVOTE:
        {
            ...
            if (type == SA_MAP && d == NULL)
                d = player1; // gonext uses this
            if (type < 0 || type >= SA_NUM || !d)
                return;
...
void setDemoFilenameFormat(char *fmt)
{
    ...
    if (fmt && fmt[0] != '\0')
    {
        ...
        copystring(demofilenameformat, DEFDEMOFILEFMT); // reset to default if passed empty string - or should we output the current value in this case?
}
...
void setDemoTimestampFormat(char *fmt)
{
    ...
    if (fmt && fmt[0] != '\0')
    {
        ...
        copystring(demotimestampformat, DEFDEMOTIMEFMT); // reset to default if passed empty string - or should we output the current value in this case?
}
