...
#define DEBUG(v)                                                                                                  \
    if (DEBUGCOND)                                                                                                \
    {                                                                                                             \
        std::cout << behindpath(__FILE__) << ":" << __LINE__ << " " << __FUNCTION__ << "(..) " << v << std::endl; \
    }
#define DEBUGVAR(v)                                                                                                              \
    if (DEBUGCOND)                                                                                                               \
    {                                                                                                                            \
        std::cout << behindpath(__FILE__) << ":" << __LINE__ << " " << __FUNCTION__ << "(..) " << #v << " = " << v << std::endl; \
    }
#else
#define DEBUG(v) \
    {            \
...
// EGK: If len = 0, an unexpected byte will be nulled.
...
#undef HTFIND
    ...
    void clear(bool del = true)
    {
        ...
        {
            for (chainchunk *chunk = chunks; chunk; chunk = chunk->next)
                insertchains(chunk);
        }
    }
};
...
template <class T>
inline void endiansame(T *buf, int len) {}
#ifdef SDL_BYTEORDER
...
template <class T>
inline T bigswap(T n) { return *(const uchar *)&islittleendian ? endianswap(n) : n; }
template <class T>
