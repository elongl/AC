...
void processevent(client *c, explodeevent &e)
{
    ...
    for (int i = 1; i < c->events.length() && c->events[i].type == GE_HIT; i++)
    {
        ...
        if (!clients.inrange(h.target))
            continue;
        client *target = clients[h.target];
        if (target->type == ST_EMPTY || target->state.state != CS_ALIVE || h.lifesequence != target->state.lifesequence || h.dist < 0 || h.dist > EXPDAMRAD)
            continue;

        int j = 1;
        for (j = 1; j < i; j++)
            if (c->events[j].hit.target == h.target)
                break;
        if (j < i)
            continue;
        ...
        if (chk_gun && chk_dir && chk_dst && chk_cnr)
            damage = INT_MAX; // nade suicide
...
void processevent(client *c, shotevent &e)
{
    ...
    if (e.gun == GUN_PISTOL && gs.akimbomillis > gamemillis)
        gs.gunwait[e.gun] /= 2;
...
#ifdef ACAC
                if (!sg_engine(target, c, numhits_c, numhits_m, numhits_o, bonusdist))
                    continue;
#endif
            }
            else
...
{
    clientstate &gs = c->state;
    if (m_mp(gamemode) && !gs.isalive(gamemillis))
        return;
...
{
    clientstate &gs = c->state;
    if (!gs.isalive(gamemillis) || gs.akimbomillis)
        return;
...
{
    int n = 1;
    while (n < c->events.length() && c->events[n].type == GE_HIT)
        n++;
    c->events.remove(0, n);
}
