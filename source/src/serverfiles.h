...
struct servermapbuffer // sending of maps between clients
{
    ...
    int available()
    {
        ...
        {
            if (!revision || (revision == smapstats.hdr.maprevision))
                return cgzsize;
        }
        return 0;
    }
    ...
    void setrevision()
    {
        if (available() && !revision)
            revision = smapstats.hdr.maprevision;
    }
    ...
    bool sendmap(const char *nmapname, int nmapsize, int ncfgsize, int ncfgsizegz, uchar *ndata)
    {
        ...
        if (!nmapname[0] || nmapsize <= 0 || ncfgsizegz < 0 || nmapsize + ncfgsizegz > MAXMAPSENDSIZE || ncfgsize > MAXCFGFILESIZE)
...
bool mapisok(mapstats *ms)
{
    ...
    if (Mopen > MAXMAREA)
    {
        logline(ACLOG_INFO, "MAP CHECK FAIL: There is a big open area in this (hint: use more solid walls)", Mheight);
        return false;
    }
    if (SHhits > MAXHHITS)
    {
    ...
    for (int i = 0; i < ms->hdr.numents; i++)
    {
        ...
        for (int j = 0; j < ms->hdr.numents; j++)
        {
            ...
            if (r2 == 0.0f)
            {
                logline(ACLOG_INFO, "MAP CHECK FAIL: Items too close %s %s (%hd,%hd)", entnames[v], entnames[w], p[0], p[1]);
                return false;
            }
            r2 = 1 / r2;
            if (r2 < 0.0025f)
        ...
        if (hdensity > 0.5f)
        {
            logline(ACLOG_INFO, "MAP CHECK FAIL: Items too close %s %.2f (%hd,%hd)", entnames[v], hdensity, p[0], p[1]);
            return false;
        }
        switch (v)
        {
#define LOGTHISSWITCH(X)                                                                                            \
    if (density > X)                                                                                                \
    {                                                                                                               \
        logline(ACLOG_INFO, "MAP CHECK FAIL: Items too close %s %.2f (%hd,%hd)", entnames[v], density, p[0], p[1]); \
        return false;                                                                                               \
    }
        case I_CLIPS:
        case I_HEALTH:
...
struct servermaprot : serverconfigfile
{
    ...
    void read()
    {
        if (getfilesize(filename) == filelen)
            return;
        configsets.shrink(0);
        if (!load())
            return;
        ...
        while (p < buf + filelen)
        {
            ...
            if (l)
            {
                ...
                for (i = 3; i < CONFIG_MAXPAR; i++)
                    c.par[i] = 0; // default values
...
#endif
        if (configsets.empty())
            fatal("maprot unavailable");
        int n = numclients();
        int csl = configsets.length();
        int ccs = curcfgset;
        if (ccs >= 0 && ccs < csl)
            ccs += configsets[ccs].skiplines;
        configset *c = NULL;
        loopi(3 * csl + 1)
        {
            ccs++;
...
struct serveripblacklist : serverconfigfile
{
    ...
    void read()
    {
        ...
        while (p < buf + filelen)
        {
            ...
            if (l[strspn(l, " ")])
            {
                for (int i = (int)strlen(l) - 1; i > 0 && l[i] == ' '; i--)
                    l[i] = '\0';
...
struct servernickblacklist : serverconfigfile
{
        ...
        while (p < buf + filelen)
        {
            ...
            if (l)
            {
                ...
                { // accept nickname IP-range
                    ...
                    if (!i)
                        i = &whitelist.access(newstring(s), -1);
                    s += strlen(s) + 1;
                    while (s < p)
                    {
    ...
    int checkwhitelist(const client &c)
    {
        ...
        if (!idx)
            return NWL_UNLISTED; // no matching entry
        int i = *idx;
        ...
        while (i >= 0)
        {
            ...
            { // check IP
                needipr = true;
                if (!cmpipmatch(&ipr, &ic.ipr))
                    iprok = true; // range match found
            }
            i = whitelistranges[i].next;
        }
        if (needpwd && !pwdok)
            return NWL_PWDFAIL; // wrong PWD
        if (needipr && !iprok)
            return NWL_IPFAIL; // wrong IP
        return NWL_PASS;
    }
    ...
    {
        if (blacklines.empty())
            return -2; // no nickname blacklist loaded
        string nameuc;
        ...
        loopv(blacklines)
        {
            loopj(MAXNICKFRAGMENTS)
            {
                int k = blacklines[i].frag[j];
                if (k < 0)
                    return blacklines[i].line; // no more fragments to check
...
struct serverinfofile
{
    ...
    {
        const char *motd;
        if (*lang && (motd = getinfocache(motdbase, lang)))
            return motd;
